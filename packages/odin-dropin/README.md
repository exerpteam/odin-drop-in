...
*   `params` (`OdinDropinInitializationParams`, **Required**): An object containing configuration parameters:
    ...
    *   `billingFieldsConfig?` (`BillingFieldsConfig`, *Optional*): An optional object to configure the visibility of additional billing fields.
        *   **Type `BillingFieldsConfig`:** `{ name?: boolean; /* ... other fields like addressLine1 ... */ }`
        *   **Example:** ` { name: true } ` to display the "Name on Card" field. If omitted, no optional billing fields are shown.
        *   *(Currently, only `name` is supported. More fields like address will be added in future updates.)*
