...
export class ExerpOdinCcForm {
  ...
  private _createOdinPayForm() {
    ...
    this.log('DEBUG', `Attempting to create form for paymentMethodType: ${this.paymentMethodType}. isSingleUse: ${this.isSingleUse}`);
    this.isLoading = true;
  ...
  private _createOdinPayCardForm() {
    this.log('DEBUG', `[Core] Creating CARD form. isSingleUse: ${this.isSingleUse}`);
  ...
  private _createOdinPayBankAccountForm() {
    ...
    try {
      this.odinPayInstance.createBankAccountForm({
        // isSingleUse: this.isSingleUse, // ⚠️ Omitted for now based on report; add back if needed.
        submitButton: {
          selector: this.getFieldId('odinSubmitButton'),
          callback: (result: any) => {
